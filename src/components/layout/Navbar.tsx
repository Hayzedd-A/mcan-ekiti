...
<div
  ...
  style={{
    background: "#0f3d22",
    clipPath: mobileOpen
      ? "circle(200% at calc(100% - 36px) 32px)"
...
>
  ...
  <div className="flex flex-col h-full">
    ...
    {/* Nav Links */}
    <nav className="px-8 pt-4 flex flex-col gap-1 overflow-y-auto">
      {navLinks.map((link, i) => (
        <Link
          key={link.href}
          href={link.href}
          onClick={() => setMobileOpen(false)}
          className="group flex items-center gap-4 py-2 border-b border-white/10 last:border-0"
          style={{
        ...
        >
          <span className="text-white text-lg font-bold group-hover:text-[#4ade80] transition-colors">
            {link.label}
          </span>
          <span className="ml-auto text-white/20 group-hover:text-[#4ade80] group-hover:translate-x-1 transition-all">
            →
          </span>
        </Link>
      ))}
 ...
 <div
    className="px-8 py-4 border-t border-white/10 mt-[60px] sm:mt-[116px]"
      style={{
        opacity: mobileOpen ? 1 : 0,
    ...
    >
      <div className="flex gap-3">
        <Link
          href="/register"
          onClick={() => setMobileOpen(false)}
          className="flex-1 text-center border-2 border-white/40 text-white px-3 py-2.5 rounded-xl text-sm font-semibold hover:border-white hover:bg-white/10 transition-all"
        >
          Register
        </Link>
        ...
        <Link
          href="/donate"
          onClick={() => setMobileOpen(false)}
          className="flex-1 flex items-center justify-center gap-2 bg-[#4ade80] text-[#0f3d22] px-3 py-2.5 rounded-xl text-sm font-bold hover:bg-[#22c55e] transition-colors"
        >
          Donate Now
        </Link>
      </div>
