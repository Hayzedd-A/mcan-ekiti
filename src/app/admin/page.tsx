...
async function getStat(endpoint: string): Promise<number> {
  try {
    const res = await fetch(`${NEXT_PUBLIC_BASE_URL}/api/${endpoint}`, {
      cache: "no-store",
    });
    if (!res.ok) return 0;
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Overview of your MCAN Ekiti website content
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        {stats.map(({ label, value, icon: Icon, href, color }) => (
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-0.5 group-hover:text-[#1B6B3A] transition-colors">
              {label}
            </p>
          </Link>
        ))}
