...
export default function AdminDonationAccountPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaved(false);
    try {
      const r = await fetch("/api/donation-account", {
        method: "PATCH",
    ...
    <div className="max-w-lg">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-0.5">
          These details are shown on the public Donate page
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        {fetching ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {saved && (
              <p className="text-sm text-[#1B6B3A] bg-[#e8f5ee] p-3 rounded-lg">
            ...
            {[
              ...
              },
              {
                label: "Account Name",
                name: "accountName",
                placeholder: "e.g. MCAN Ekiti State Branch",
              },
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#1B6B3A] text-white py-3 rounded-lg text-sm font-semibold hover:bg-[#154a2a] transition-colors disabled:opacity-60"
            >
