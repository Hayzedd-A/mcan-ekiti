...
type F = { name: string; position: string; title?: string; order: string };
const EMPTY: F = { name: "", position: "", title: "", order: "0" };
...
export default function AdminLeadershipPage() {
  ...
  const handleChange =
    (k: keyof F) => (ev: React.ChangeEvent<HTMLInputElement>) =>
      ...
      {leaders.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {leaders.map((leader) => (
            <div
              key={leader._id}
              className="bg-white rounded-xl border border-gray-100 overflow-hidden text-center p-4 hover:shadow-sm transition-shadow"
            >
              ...
              {leader.title && (
                <p className="text-xs text-gray-500 mt-1">{leader.title}</p>
              )}
