...
async function getLodges(): Promise<ILodge[]> {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:5321"}/api/lodges`,
      { cache: "no-store" }
    );
    if (!res.ok) return [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {lodges.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {lodges.map((lodge) => (
              <div key={lodge._id} className="bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-md transition-shadow">
                <div className="relative h-48">
                  <Image src={lodge.image || FALLBACK_IMG} alt={lodge.title} fill className="object-cover" />
                  <span className={`absolute top-3 right-3 text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_STYLES[lodge.status] ?? "bg-gray-200 text-gray-600"}`}>
                    {lodge.status}
                  </span>
                </div>
                <div className="p-5">
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed mb-4">{lodge.description}</p>
                  <Link
                    href="/contact"
